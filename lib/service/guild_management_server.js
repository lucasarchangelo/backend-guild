    ...
    login: function (req, res, next) {
        ...
        if (!token) {
            $database.findUserByEmail(req.body.email, (err, user) => {
                ...
                const token = jwt.sign({ id: user._id,  nome: user.nome, email: user.email, idpsn: user.idpsn, acesso: user.acesso }, $config.secret, { expiresIn: '7d' });
            ...
            jwt.verify(token, $config.secret, (err, decodedToken) => {
                ...
                $database.findUserByEmail(decodedToken.email, (err, user) => {
                    if (err) {
                    return next(new restifyErrors.InternalServerError('Error on the server.'));
                    }
                    if (!user) {
