...
const Database = {
    ...
    listByGame: function (listBody, callback) {
      logger.debug(listBody);
        Pendency.find({ game: listBody.game},{},{sort:{ name: -1}}, (err, pendencies) => {
          if (err) {
    ...
    listByGameWeekly: function (listBody, callback) {
      logger.debug(listBody);
        Pendency.find({ game: listBody.game, weekly: listBody.weekly },{},{sort:{ name: -1}}, (err, pendencies) => {
          if (err) {
            logger.error('[listAll] error: ' + err);
            return callback(`ERROR=${err}`);
          }
